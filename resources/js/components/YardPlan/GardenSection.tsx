...
import { GardenProductCard } from './ProductCard';
    ...
    return (
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex items-center justify-between hover:border-green-200 transition-colors group">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-xl font-bold text-gray-900">{index}. Add garden care</h2>
        </div>
        <button onClick={onOpenModal} className="bg-green-700 text-white px-6 py-2 rounded-lg font-bold hover:bg-green-800 transition-colors shadow-sm">
          Start
        </button>
      </div>
    );
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
      <div className="p-6 border-b border-gray-100 flex items-center justify-between bg-white">
        <div className="flex items-center gap-3">
          ...
          <h2 className="text-xl font-bold text-gray-900">{index}. Your garden care plan</h2>
        </div>
        <div className="flex items-center gap-3">
          <button onClick={onOpenModal} className="flex items-center gap-1.5 text-xs font-bold text-gray-400 hover:text-green-700 transition-colors">
            <Pencil size={13} /> Edit
          </button>
      ...
      <div className={`transition-all duration-300 ${gardenEnabled ? 'opacity-100' : 'opacity-50 grayscale'}`}>
        ...
        <div className="p-6 space-y-4 bg-gray-50/30">
          {gardenFeatures.map((feature, i) => (
            <GardenProductCard key={feature.title} feature={feature} item={gardenItems[i] ?? null} index={i} />
          ))}
        </div>
      </div>
    </div>
