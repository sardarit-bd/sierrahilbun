...
const GardenQuizModal = ({ isOpen, onClose }) => {
  ...
  useEffect(() => {
    if (isOpen) { setStep(1); setGardenTypes([]); setGardenSize(null); setErrors({}); setIsSubmitting(false); }
  }, [isOpen]);
  ...
  const handleNext = () => {
    if (gardenTypes.length === 0) { setErrors({ garden_types: 'Please select at least one garden type.' }); return; }
    setStep(2);
  };

  const handleSubmit = () => {
    if (!gardenSize) { setErrors({ garden_size: 'Please select a garden size.' }); return; }
    setIsSubmitting(true);
    router.post(route('yard.garden-quiz'), { garden_types: gardenTypes, garden_size: gardenSize }, {
      preserveScroll: true,
      onSuccess: () => { onClose(); },
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 bg-orange-100 rounded-full flex items-center justify-center text-orange-700"><Flower size={20} /></div>
            <div>
                ...
                {GARDEN_TYPES.map((type) => {
                  ...
                  return (
                    <button key={type.value} onClick={() => toggleType(type.value)}
                      className={`w-full flex items-center justify-between px-4 py-3 rounded-xl border-2 transition-all duration-150 text-left ${isSelected ? 'border-green-600 bg-green-50 text-green-900' : 'border-gray-200 bg-white text-gray-700 hover:border-gray-300'}`}>
            ...
            <div>
              ...
              <div className="space-y-2">
                {GARDEN_SIZES.map((size) => {
                  ...
                  return (
                    <button key={size.value} onClick={() => { setGardenSize(size.value); setErrors(prev => ({ ...prev, garden_size: null })); }}
                      className={`w-full flex items-center justify-between px-4 py-3 rounded-xl border-2 transition-all duration-150 text-left ${isSelected ? 'border-green-600 bg-green-50 text-green-900' : 'border-gray-200 bg-white text-gray-700 hover:border-gray-300'}`}>
                      <div>
        ...
        <div className="px-6 pb-6 flex items-center justify-between gap-3">
          ...
          {step === 1 && (
            <button onClick={handleNext} className="ml-auto bg-green-700 text-white px-6 py-2.5 rounded-lg font-bold text-sm hover:bg-green-800 transition-colors shadow-sm">Next</button>
          )}
          {step === 2 && (
