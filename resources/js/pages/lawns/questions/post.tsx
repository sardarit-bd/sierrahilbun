...
const QUESTIONS = [
  ...
  },
  {
    ...
    options: [
      ...
      { id: 'not_much', label: "Not much", img: "https://images.unsplash.com/photo-1589923188900-85dae523342b?auto=format&fit=crop&q=80&w=400" } // Generic outdoor/picnic vibe
    ]
  },
  {
    id: 'knowledge',
  ...
  },
  {
    ...
    hasMore: true,
    options: [
      { id: 'fescue', label: "Fescue", tag: "MOST COMMON NEAR YOU", img: "https://images.unsplash.com/photo-1533460004989-acf6be3fab53?auto=format&fit=crop&q=80&w=400" },
  ...
  },
  {
    ...
    options: [
      { id: 'liquid', label: "Liquid hose-on fertilizer", recommended: true, desc: "You'll love our innovative liquid fertilizers for fastest results with easiest precision. All you need is a hose and about 10 minutes per application.", img: "https://images.unsplash.com/photo-1622383563227-044011358d16?auto=format&fit=crop&q=80&w=400" },
      { id: 'granular', label: "Dry granular fertilizer", img: "https://images.unsplash.com/photo-1611735341450-74d61e66ee69?auto=format&fit=crop&q=80&w=400" }
    ]
  }
];
  ...
  return (
    <div 
      ...
      className={`
        ...
        ${selected 
          ? 'border-green-600 shadow-lg ring-1 ring-green-600 transform scale-[1.01]' 
      ...
      `}
    >
      {/* Badge for recommendations or tags */}
      {(option.tag || option.recommended) && (
        <div className={`
          absolute top-0 left-0 z-20 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider
          ${option.recommended ? 'bg-yellow-400 text-yellow-900 top-4 left-4 rounded-sm shadow-sm' : 'bg-green-600 text-white rounded-br-lg'}
        `}>
      ...
      {selected && (
        <div className={`
          ...
          ${expandedView 
            ? 'bottom-4 left-4' 
            : 'left-4 top-1/2 -translate-y-1/2' // Centered vertically for horizontal cards
          }
          ${option.recommended && expandedView ? 'bottom-6' : ''}
        `}>
          <div className="rounded-full flex items-center justify-center transition-all duration-200 shadow-sm bg-green-600 w-8 h-8">
      ...
      <div className={`
        ...
        ${expandedView ? 'p-6 pt-4' : 'px-4 py-2 pl-12'} 
      `}>
...
export default function Questionnaire() {
  ...
  const handleSelect = (optionId) => {
    ...
    // Only auto-advance if it's NOT the last step
    if (currentStep < QUESTIONS.length - 1) {
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          setShowMoreGrass(false); // Reset expander
        }, 300);
      }, 400); // 400ms delay for visual confirmation
    } 
    // If it is the last step, we do nothing but update the state, 
    // so the "Submit" button can be rendered.
  };
  ...
  const handleSubmit = () => {
    // In a real app, you would submit `answers` to your backend here
    alert("Questionnaire Complete! Redirecting to your plan...");
  ...
  // Logic to merge standard and 'more' options if expanded
  ...
  // Check if we are on the last step and have an answer
  ...
  return (
    <AppHeaderLayout>
      ...
      <div className="flex-grow flex flex-col items-center justify-start pt-12 pb-20 px-4 sm:px-6 animate-fade-in">
        ...
        <div className="w-full max-w-2xl relative mb-8">
          {currentStep > 0 && (
            <button 
              onClick={handleBack}
              className="absolute -top-10 left-0 text-green-700 font-bold flex items-center gap-1 text-sm hover:underline transition-all"
            >
          ...
          <h1 className={`
            text-3xl sm:text-4xl font-extrabold text-gray-900 text-center leading-tight transition-all duration-300 transform
        ...
        {/* Options Grid */}
        <div className={`
          w-full max-w-2xl grid gap-4 transition-all duration-300 transform
          ${isAnimating ? 'opacity-0 translate-y-4' : 'opacity-100 translate-y-0'}
          ${isExpandedLayout ? 'grid-cols-1' : 'grid-cols-1'} 
        `}>
        ...
        {/* Expandable Section for Grass Type */}
        {currentQuestion.hasMore && !showMoreGrass && (
          <button 
            onClick={() => setShowMoreGrass(true)}
            className="mt-6 flex items-center gap-2 px-6 py-3 bg-white border border-green-600 text-green-700 font-bold rounded-full hover:bg-green-50 transition-all shadow-sm active:scale-95"
          >
        ...
        {canSubmit && (
          <div className="mt-8 w-full max-w-2xl animate-fade-in-up">
            <button
              className="w-full bg-green-700 hover:bg-green-800 text-white font-bold py-4 rounded-xl shadow-lg transition-all transform hover:scale-[1.01] active:scale-[0.98] text-lg flex items-center justify-center gap-2"
            >
              <Link href="/yard/plan">Get my lawn plan</Link>
            </button>
          </div>
        )}
