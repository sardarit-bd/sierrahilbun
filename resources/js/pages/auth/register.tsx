...
// Custom styled components (keeping your original design)
const InputError = ({ message, className }: { message?: string; className?: string }) =>
    message ? <p className={`text-sm text-red-600 font-medium ${className}`}>{message}</p> : null;

const Label = ({ children, htmlFor }: { children: React.ReactNode; htmlFor: string }) => (
    <label htmlFor={htmlFor} className="block text-sm font-bold text-gray-700 mb-1.5 uppercase tracking-wide text-xs">
        {children}
    </label>
);
...
const Input = ({ className = '', ...props }: React.InputHTMLAttributes<HTMLInputElement>) => (
    <input
        className={`flex h-12 w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-900 placeholder:text-gray-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#2E7D32] focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 transition-all duration-200 ${className}`}
        {...props}
    />
);
...
const Button = ({ children, className = '', disabled, variant = 'primary', ...props }:
    ...
    const base = "inline-flex items-center justify-center rounded-xl text-sm font-bold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-950 focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 h-12 px-8 active:scale-95";
    const variants = {
    ...
    <AppHeaderLayout>
        ...
        <div className="flex min-h-[calc(100vh-80px)]">
            <div className="hidden lg:flex lg:w-1/2 relative bg-[#1B5E20] overflow-hidden">
                <img
                    src="https://images.unsplash.com/photo-1668189777890-495c36095340?q=80&w=1170&auto=format&fit=crop"
                    alt="Lush Green Lawn"
                ...
                <div className="absolute bottom-0 left-0 right-0 p-16 bg-gradient-to-t from-black/80 via-black/40 to-transparent text-white">
                    <h2 className="text-4xl lg:text-5xl font-black font-serif mb-6 leading-[1.1]">
                        Join the future <br /> of lawn care.
                    </h2>
                    <p className="text-lg text-white/90 font-medium max-w-md leading-relaxed">
                        Create your free account today and get a custom plan tailored specifically to your soil and climate.
                    </p>
                </div>
            </div>
            ...
            <div className="w-full lg:w-1/2 flex items-center justify-center p-6 bg-[#FDFBF7]">
                <div className="w-full max-w-md space-y-8">
                    <div className="text-center lg:text-left">
                        <h1 className="text-3xl md:text-4xl font-black text-[#1A1A1A] font-serif mb-3">{title}</h1>
            ...
            <form onSubmit={submit} className="flex flex-col gap-5">
                <div className="grid gap-5">
                    ...
                    <div className="grid gap-2">
                        ...
                        <Input
                            ...
                            className={errors.name ? "border-red-500 focus-visible:ring-red-500 bg-red-50/50" : ""}
                        />
                    ...
                    <Button type="submit" className="mt-2 w-full text-base py-6" tabIndex={5} disabled={processing}>
                ...
                <div className="relative my-4">
                    ...
                    <div className="relative flex justify-center text-xs uppercase">
                        <span className="bg-[#FDFBF7] px-2 text-gray-500 font-bold tracking-wider">Or sign up with</span>
                    </div>
                </div>
                ...
                <div className="text-center mt-6 pt-4 border-t border-gray-100">
                    <p className="text-gray-500 text-sm">
                        ...
                        <Link href={route('login')} tabIndex={6} className="font-bold text-lg block mt-1 text-[#2E7D32]">
                            Log in
                        </Link>
                    </p>
                </div>
